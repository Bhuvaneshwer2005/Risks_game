import { ALL_TERRITORY_IDS, areAdjacent, type TerritoryId } from "./mapData.ts";
import {
  maxAttackerDice,
  maxDefenderDice,
  moveInBounds,
...
import { calculateReinforcements, territoryCount } from "./reinforcements.ts";
import type { Card, GameState, PlayerId, PlayerState } from "./types.ts";
import { currentPlayerId } from "./types.ts";
...
export function applyAttack(
  ...
  if (defenderArmiesAfter <= 0) {
    ...
    territories[to] = { owner: attacker, armies: 0 };
    if (defenderId) {
